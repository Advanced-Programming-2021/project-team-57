...
public class Deck {
    ...
    private static ArrayList<Card> traps;
	...
	public Deck() throws IOException, UnexpectedFormatException
	{
		...
		if(monsters==null || spells ==null || traps==null)
		{
			    monsters = loadCardsFromFileHandler(monstersPath);
			    loadingMonsters=false;
				spells = loadCardsFromFileHandler(spellsPath);
				traps = loadCardsFromFileHandler(trapsPath);
		}
		buildDeck(monsters, spells,traps);
		shuffleDeck();
	}
	...
	{
		ArrayList<Card> cards = new ArrayList<Card>(60);
		BufferedReader br = new BufferedReader(new FileReader(path));
		int line = 1;
		while(br.ready())
		{ 
			String [] st = br.readLine().split(",");
			if((!(st[0].equals("Spell"))) && (!(st[0].equals("Monster"))) && (!(st[0].equals("Monster"))))
				throw new UnknownCardTypeException(path, line, st[0]);

			if(loadingMonsters && st.length!=3 || !loadingMonsters && st.length!=6)
	...
	private void buildDeck(ArrayList<Card> monsters, ArrayList<Card> spells, ArrayList<Card> traps)
	{
		...
		for(int i = 0; i < 5; i++)
		{
			trapsCard traps = (trapsCard)((trapsCard)(traps.get((new Random()).nextInt(traps.size())))).clone();
			deck.add(traps);
		}
	}
